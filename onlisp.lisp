...
(defun mapcars2 (fn &rest lsts)
  (let ((result nil))
    (dolist (lst lsts)
      (let ((acc nil))
        (dolist (obj lst (push (nreverse acc) result))
          (push (funcall fn obj) acc))))
    (nreverse result)))
...
(in-if #'(lambda (y) (equal 3 y)) 1 3 4 5)
...
(case (intern (concatenate 'string "hello" "-" "world"))
  (ciao-world "What is that")
...
(defmacro for-capture ((var start stop) &body body)
  `(do ((,var ,start (1+ ,var))
        (limit ,stop))
       ((> ,var limit))
     ,@body))
